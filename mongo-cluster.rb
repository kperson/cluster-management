...
cluster = Cluster.load_from_file('cluster.yml')
...
def is_master(port)
  ...
  system "mongo --port %s --eval '%s' > %s" % [port, FileHelpers.read_file_at('mongo-scripts/is_master.js'), file_out]
...
def configure_replica(mongo_endpoints, username, password, port)
  ...
  system "mongo --port %s --eval '%s'" % [port, Mustache.render(template, bind)]
end
